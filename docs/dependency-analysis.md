...
- polycpp capability snapshot consumed: yes. polycpp checkout `master` snapshot dated 2026-04-23 confirms `polycpp::process::getenv`, `polycpp::Math::round/floor/max`, `polycpp::Number::parseInt`/`Number::isNaN`, and `polycpp::Error` exist; recorded in `docs/research.md` `## Polycpp ecosystem reuse analysis`.
