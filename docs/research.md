...
- polycpp reuse opportunity not yet realised: `polycpp::platform::isTerminal(int fd)` cleanly wraps `isatty`/`_isatty` cross-platform. The existing companion implementation in `detail/supports_color.hpp` rolls its own `#ifdef _WIN32 ... isatty_impl` because the companion predates `polycpp::platform`. Switching to `polycpp::platform::isTerminal` would let the companion drop the conditional include and the `#define isatty_impl`. Recorded as audit finding `AF-2026-05-01-A` in `docs/divergences.md`; not changed in this catch-up run.
