...
- polycpp core paths inspected: `include/polycpp/core/error.hpp`, `include/polycpp/core/math.hpp`, `include/polycpp/core/number.hpp`, `include/polycpp/process.hpp`. These are the primitives the port actually depends on.
- polycpp capability snapshot: base polycpp checked out at `master` on 2026-04-23; `polycpp::process::getenv`, `polycpp::Math::round/floor/max`, `polycpp::Number::parseInt`/`Number::isNaN`, and `polycpp::Error` are all available in that snapshot.
- transport/listener capability review: not applicable. chalk has no socket, listener, TLS, or stream transport surface; the C++ port consumes a raw POSIX/Windows file descriptor only for `isatty`. No deferred socket or TLS work exists.
- polycpp core types/functions selected: `polycpp::process::getenv` (env access), `polycpp::Number::parseInt`/`Number::isNaN` (FORCE_COLOR / TERM_PROGRAM_VERSION parsing), `polycpp::Math::round`/`Math::floor`/`Math::max` (color downsampling), `polycpp::Error` (level validation throw)
- polycpp core types/functions rejected: none rejected; chalk does not need `Buffer`, `Stream`, `Headers`, `URL`, `JSON`, or `events`
