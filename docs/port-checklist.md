...
This repo predates libgen. The planning artifacts below were filled in
retroactively as part of the libgen catch-up review described in the
`Release gate` section of `docs/porting-playbook.md`. Implementation,
documentation, and public release happened before strict readiness was
written down, so the strict pre-coding gate is informational here; the
relevant post-implementation gate is `scripts/check-port-validation.py`.
...
- GitHub repo URL: https://github.com/polycpp/chalk
- GitHub repo private: no
- GitHub repo visibility note: this companion was already promoted to public before the libgen catch-up review; see `Public Release` below
...
- [-] strict readiness passed (informational only; this repo predates libgen)
